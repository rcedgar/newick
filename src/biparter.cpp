...
void BiParter::Init(const TreeN &InputTree)
	{
	m_Labels.clear();
	...
	for (uint Index = 0; Index < LeafCount; ++Index)
		...
		if (m_LabelToIndex.find(Label) != m_LabelToIndex.end())
			Die("BiParter::Init(), dupe label >%s", Label.c_str());
	...
	for (uint Node = 0; Node < NodeCount; ++Node)
		...
		m_T.AppendSubtreeLeafNodes(Node, SubtreeLeafNodes);
...
uint BiParter::GetHash(const vector<bool> &Part) const
	{
...
uint BiParter::GetHash(uint Node) const
	{
	const uint SlotCount = SIZE(m_HashTable);
...
uint BiParter::GetLabelIndex(const string &Label) const
	{
	if (Label.empty())
		return UINT_MAX;
	map<string, uint>::const_iterator p = m_LabelToIndex.find(Label);
...
uint BiParter::Search(const vector<bool> &Part) const
	{
	if (Part.empty())
		return UINT_MAX;
	uint h = GetHash(Part);
...
uint BiParter::GetOtherCount(const vector<bool> &Part,
...
void BiParter::SearchBestMatch(const vector<string> &Labels,
...
void BiParter::ToTSV(const string &Name, FILE *f) const
	{
	if (f == 0)
		return;
	uint LabelCount = SIZE(m_Labels);
	const uint NodeCount = m_T.GetNodeCount();
	uint Root = m_T.GetRoot();
	for (uint Node = 0; Node < NodeCount; ++Node)
		{
		const vector<bool> &Part = m_PartVec[Node];
...
void BiParter::LogPatterns() const
	{
	Log("\n");
	const uint NodeCount = m_T.GetNodeCount();
	uint Root = m_T.GetRoot();
...
void BiParter::CountFeatures(const FeatureTable &FT,
  vector<uint> &ValueIndexToTotal,
  vector<vector<uint> > &NodeToValueCounts)
	{
	ValueIndexToTotal.clear();
	NodeToValueCounts.clear();

	const uint LabelCount = SIZE(m_Labels);
	const uint PartCount = SIZE(m_PartVec);
	const uint NodeCount = m_T.GetNodeCount();
	const uint ValueCount = FT.GetValueCount();

	ValueIndexToTotal.resize(ValueCount, 0);
	vector<uint> LabelIndexToValueIndex;
	for (uint LabelIndex = 0; LabelIndex < LabelCount; ++LabelIndex)
		...
		LabelIndexToValueIndex.push_back(ValueIndex);
		if (ValueIndex == UINT_MAX)
			continue;

		assert(ValueIndex < ValueCount);
		ValueIndexToTotal[ValueIndex] += 1;
		}

	asserta(PartCount == NodeCount);
	NodeToValueCounts.resize(PartCount);
	for (uint Node = 0; Node < NodeCount; ++Node)
		{
		if (m_T.IsRoot(Node) || m_T.IsLeaf(Node))
			continue;
		...
		vector<uint> &ValueCounts = NodeToValueCounts[Node];
		ValueCounts.clear();
		...
		for (uint LabelIndex = 0; LabelIndex < LabelCount; ++LabelIndex)
			...
			if (!Part[LabelIndex])
				continue;
			uint ValueIndex = LabelIndexToValueIndex[LabelIndex];
			if (ValueIndex < UINT_MAX)
				{
				assert(ValueIndex < ValueCount);
...
void BiParter::GetPartInternalNodes(uint PartIndex, vector<uint> &Nodes) const
	{
	Nodes.clear();
...
void BiParter::GetPartInternalNodeLabels(uint PartIndex, vector<string> &Labels) const
	{
	Labels.clear();
	if (m_T.IsRoot(PartIndex) || m_T.IsLeaf(PartIndex))
		return;
...
void cmd_biparts()
	{
	const string &InputFileName = opt(biparts);
